...
# 限制长度
def _limit_str_len_(input_str,step_len):
...
def limit_str_len(input_str,step_len):
    fstr ='\n'
    ...
    for ind in range(all_num):
        ...
        if step_loc-start_num>step_len:
            new_str=new_str+_limit_str_len_(input_str[start_num:step_loc],step_len)
        else:
...
def add_newline_strlist(input_str,strlist):
 ...
 # 对数字加., 前面添加换行
def add_newline_digit(input_str):
